...
local gears = require("gears")       -- Additional stuff
...
local portage_info_w = {} -- Portage info widget
--}}}
...
-- Portage info widget
portage_info_w.widget = wibox.widget.textbox()
portage_info_w.timer = timer{timeout = utils.PORTAGE_INFO_W_TIMEOUT}
portage_info_w.update = function ()
    ...
    if status_line_of then
         status_line = string.format("Portage: <span color='%s'>%s</span> (%s)",
                                    theme.PURPLE_COLOR, status_line, status_line_of)
    else
        status_line = string.format("Portage: <span color='%s'>Not installing</span>",
                                    theme.PURPLE_COLOR, status_line)
    end
    portage_info_w.widget:set_markup(status_line)
end

portage_info_w.update()
portage_info_w.timer:connect_signal("timeout", portage_info_w.update)
portage_info_w.timer:start()
--}}}
...
net_usage_w.update = function ()
    ...
    for line in io.lines("/proc/net/dev") do
        if line:find(utils.NETWORK_INTERFACE) then
...
net_usage_w.widget:set_font(theme.WIBOX_TOP_FONT)
net_usage_w.update()
...
cpu_usage_w.widget:set_font(theme.WIBOX_TOP_FONT)
cpu_usage_w.update()
...
mpd_info_w.update = function ()
    ...
    elseif mpd_info_w.status["state"] == "stop" then
        ...
        artist = mpd_info_w.curr_song["Artist"] or "Unknown"
		title = mpd_info_w.curr_song["Title"] or "Unknown"
		time.gone = {}
        time.total = {}
...
cli_w.show = function ()
    awful.prompt.run({prompt = ">>> ", font = theme.WIBOX_BOT_FONT},
                    cli_w.widget,
                    function (cmd)
                        if cmd:sub(0, 1) == "!" then
...
uptime_w.widget:set_font(theme.WIBOX_BOT_FONT)
uptime_w.update()
...
tag_list = awful.widget.taglist(1, function () return tags end,
                                tag_list.buttons,
                                {font = theme.WIBOX_TOP_FONT})
--}}}
...
mem_usage_w.widget:set_font(theme.WIBOX_BOT_FONT)
mem_usage_w.init()
mem_usage_w.update()
...
temp_w.widget:set_font(theme.WIBOX_BOT_FONT)
temp_w.init()
temp_w.update()
...
disk_usage_w.widget:set_font(theme.WIBOX_BOT_FONT)
disk_usage_w.init()
disk_usage_w.update()
...
usb_usage_w.widget:set_font(theme.WIBOX_BOT_FONT)
usb_usage_w.update()
...
wibox_top = awful.wibox({
    position = "top",
    screen = 1,
    height = theme.WIBOX_TOP_H,
    bg = theme.WIBOX_TOP_BG,
    fg = theme.WIBOX_TOP_FG
})
...
wibox_bot = awful.wibox({
    position = "bottom",
    screen = 1,
    height = theme.WIBOX_BOT_H,
    bg = theme.WIBOX_BOT_BG,
    fg = theme.WIBOX_BOT_FG
})
